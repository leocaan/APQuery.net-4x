...
namespace QueryFramework.Tests.Query
{
   ...
   public class Select_Function_Test
   {
      ...
      [TestMethod]
      public virtual void Cast_Expr()
      {
         var t = CrmDbDef.department;


         {
            var castExpr = SqlExpr.Cast(t.DepartmentId, typeof(string));

            APQuery
               .select(t.DepartmentId, castExpr.As("idstring"))
               .from(t)
               .print("SELECT CAST(column_name AS nvarchar)");
         }
      }
